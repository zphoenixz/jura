...
| v3.5 | 2026-04-20 | Upgraded OpenViking 0.2.15 → 0.3.9. Memory V2 (YAML-templated memories under `entities/`, `events/`, `patterns/`, etc.) is now the active format; MCP bridge's dynamic root discovery already handles the new paths. Existing vector collections are auto-backfilled with embedding metadata on first boot — no reingest required. Hook hardening in `ov_memory.py`: `detect_backend` now raises `ServerUnreachable` instead of silently falling back to a local workspace when the configured OV server is down (previously materialized a ghost `./data/` dir at project root and diverted memory writes away from the server); `cmd_session_start` catches the error, writes `{"active": false, "last_error": ...}` so subsequent hooks no-op cleanly, and surfaces a visible status. Pure-local default path moved from `./data` to `.openviking/data-local` |
